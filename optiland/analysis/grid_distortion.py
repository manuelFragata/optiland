...
class GridDistortion:
    ...
    def view(self, figsize=(7, 5.5)):
        ...
        ax.plot(data['xp'], data['yp'], 'C1', linewidth=1)
        ax.plot(data['xp'].T, data['yp'].T, 'C1', linewidth=1)

        ax.plot(data['xr'], data['yr'], 'C0P')
        ax.plot(data['xr'].T, data['yr'].T, 'C0P')
    ...
    def _generate_data(self):
        ...
        max_field = be.sqrt(be.array(2.0)) / 2
        ...
        if self.distortion_type == 'f-tan':
            const = (self.optic.surface_group.y[-1, 0] /
                     (be.tan(1e-10 * be.deg2rad(self.optic.fields.max_field))))
            xp = const * be.tan(Hx * be.deg2rad(self.optic.fields.max_field))
            yp = const * be.tan(Hy * be.deg2rad(self.optic.fields.max_field))
        elif self.distortion_type == 'f-theta':
            const = (self.optic.surface_group.y[-1, 0] /
                     (1e-10 * be.deg2rad(self.optic.fields.max_field)))
            xp = const * Hx * be.deg2rad(self.optic.fields.max_field)
        ...
        data['xr'] = be.reshape(self.optic.surface_group.x[-1, :],
                                (self.num_points, self.num_points))
        data['yr'] = be.reshape(self.optic.surface_group.y[-1, :],
                                (self.num_points, self.num_points))
        ...
        # Find max distortion
        delta = be.sqrt((data['xp'] - data['xr'])**2 +
                        (data['yp'] - data['yr'])**2)
    ...
    def _prepare_data(self):
        ...
        """
        data = {}
        data['xr'] = be.to_numpy(self.data['xr'])
        data['yr'] = be.to_numpy(self.data['yr'])
        data['xp'] = be.to_numpy(self.data['xp'])
        data['yp'] = be.to_numpy(self.data['yp'])
        data['max_distortion'] = be.to_numpy(self.data['max_distortion'])
        return data
