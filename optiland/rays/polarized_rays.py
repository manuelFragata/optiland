...
class PolarizedRays(RealRays):
    ...
    def get_output_field(self, E):
        ...
        """
        return be.matrix_vector_multiply_and_squeeze(self.p, E)
    ...
    def update_intensity(self, state: PolarizationState):
        ...
        if state.is_polarized:
            ...
            self.i = (be.sum(be.abs(E1_x)**2, axis=1) +
                      be.sum(be.abs(E1_y)**2, axis=1)) * self._i0 / 2
    ...
    def _get_3d_electric_field(self, state: PolarizationState):
        ...
        E = (state.Ex * be.exp(1j * state.phase_x) * s +
